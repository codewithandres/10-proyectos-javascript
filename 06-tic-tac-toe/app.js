...
const renderBoard = () => {
	const html = board.map(row => {
		const cells = row.map(cell => `<button class="cell">${cell}</button>`);
...
const PCPlaysV2 = () => {
	...
	if (pcSolutions.length > 0) {
		...
		for (let i = 0; i < pcSolutions.length; i++)
			if (pcSolutions[i].level < min) min = pcSolutions[i].level;
...
const processNode = (root, nturn, level) => {
...
const createChild = (node, i, j, nturn, level) => {
